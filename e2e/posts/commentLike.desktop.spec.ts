...
test("desktop: user can like a reply", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 900 });

  //
  // 1. Login as TEST (he will like the reply later)
  //
  ...
  // 2. Create a post
  //
  const title = `Reply Like Desktop ${uniqueSuffix()}`;
  ...
  //
  // 4. Logout TEST (frontend + backend)
  //
  await page.getByRole("button", { name: "Go to login page" }).click();

  //
  // 5. Login as JOHN (reply author)
  //
  ...
  //
  // 8. Logout JOHN (frontend + backend)
  //
  await page.getByRole("button", { name: "Go to login page" }).click();

  //
  // 9. Login as TEST (liker)
  //
  await loginUI(page, "test@gmail.com", "12345678");
